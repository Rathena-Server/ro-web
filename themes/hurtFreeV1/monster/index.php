...
<table class="table is-responsive">
	<thead>
		<tr>
			...
			<th scope="col" colspan="2"><?php echo $paginator->sortableColumn('kro_name', 'kRO Name') ?></th>
		...
		<th scope="col"><?php echo $paginator->sortableColumn('base_exp', 'Base EXP') ?></th>
		<th scope="col"><?php echo $paginator->sortableColumn('job_exp', 'Job EXP') ?></th>
	...
	<?php foreach ($monsters as $monster): ?>
		...
		<?php if ($monsterImg=$this->monsterImage($monster->monster_id)): ?>
			<td data-label="Image"><img src="<?php echo htmlspecialchars($monsterImg) ?>?nocache=<?php echo rand() ?>" style="max-width:50px; max-height:50px;" /></td>
		...
		<?php else: ?>
			<td colspan="2" data-label="kRO Name">
				<?php if ($monster->mvp_exp): ?>
				<span class="mvp">MVP!</span>
				<?php endif ?>
				<?php echo htmlspecialchars($monster->kro_name ?: '') ?>
		...
		<td data-label="Element"><?php echo Flux::elementName($monster->element) ?> (Lv <?php echo floor($monster->element_level) ?>)</td>
		<td data-label="Base EXP"><?php echo number_format($monster->base_exp * $server->expRates['Base'] / 100) ?></td>
		<td data-label="Job EXP"><?php echo number_format($monster->job_exp * $server->expRates['Job'] / 100) ?></td>
